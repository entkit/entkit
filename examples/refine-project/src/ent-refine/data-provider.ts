...
export const generateOrderQuery = (resource: string, sort?: CrudSorting): undefined|OrderQuery => {
    if (sort && sort.length > 0) {
        ...
        const fieldGroupName :keyof typeof SorterEnums = resource+"SorterEnums" as any
    ...
    if (filters) {
        filters.map((filter) => {
            if (
                ...
                if (operator === "eq") {
                    queryFilters[camelCase(`${field}`, {preserveConsecutiveUppercase: true})] = value;
                } else {
                    queryFilters[camelCase(`${field}_${operator}`, {preserveConsecutiveUppercase: true})] = value;
                }
            } else {
                ...
                value.map((val) => {
                    if (val.operator === "eq") {
                        orFilters.push({
                            [ camelCase(`${val.field}`, {preserveConsecutiveUppercase: true})]: val.value,
                        });
                    } else {
                        orFilters.push({
                            [ camelCase(`${val.field}_${val.operator}`, {preserveConsecutiveUppercase: true})]: val.value,
                        });
                    }
                });
        ...
        update: async({ resource, id, variables, metaData }) => {
            ...
            for (const key in variables){
                if(key.endsWith("IDs")){
                    ...
                    cleanVars[camelCase("clear-"+key)] = true
                } else{
                    cleanVars[key] = variables[key]
                }
            }
